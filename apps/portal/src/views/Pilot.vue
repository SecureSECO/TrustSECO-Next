<template>
  <main class="pilot">
    ...
    <template v-if="state">
      ...
      <div class="summary"><article><strong>{{ state.members.length }}</strong><span>Admitted contributors</span></article><article><strong>{{ verifiedCount }}</strong><span>Closed rounds with agreement</span></article><article><strong>{{ state.ledger.finalizedHeight }}</strong><span>Finalized block · {{ state.ledger.height }} recorded</span></article><article><strong>{{ state.treasury }}</strong><span>TrustCOIN available in treasury</span></article></div>
      <p class="note" v-if="state.members.length < 3">This network needs at least three independent contributors to verify findings. With fewer contributors, observations remain unverified and bounties are refunded after closure and the 24-hour review period.</p>
      <section id="work"><h2>Observations &amp; trust scores</h2><p>These collectors measure the repository today. A version label associates the finding with a package; it does not reconstruct that version’s historical repository statistics.</p>
        ...
        <article class="round" v-for="round in [...state.rounds].reverse()" :key="round.id">
          ...
          <p>{{ metric(round.metric) }} · {{ round.observations.length }} observations · {{ round.escrow.bounty }} TrustCOIN reserved</p>
      ...
      <section id="payouts"><h2>Most recent payouts</h2><p>TrustCOIN moves from reserved bounties to contributors’ ledger balances after a 24-hour review period. Unresolved work and division remainders return to the sponsor.</p><p v-if="!payouts.length" class="empty">No payouts recorded yet.</p><div class="table-wrap" v-else><table><thead><tr><th>Contributor</th><th>TrustCOIN</th><th>Kind</th><th>Round</th><th>Ledger</th></tr></thead><tbody><tr v-for="p in payouts" :key="p.id"><td>{{ p.uid }}</td><td>{{ p.amount }}</td><td>{{ p.kind }}</td><td>{{ p.round }}</td><td>{{ p.height <= state.ledger.finalizedHeight ? '✓ Finalized' : 'Recorded' }}</td></tr></tbody></table></div></section>
      <section id="community"><h2>Community</h2><p>Admission checks a GitHub account’s age and published signing key. An operator also attests that the contributor is independent. Account age alone does not prevent one person from controlling several accounts.</p><div class="members"><article v-for="m in state.members" :key="m.id"><strong v-if="state.testNetwork">{{ m.id }}</strong><a v-else :href="'https://github.com/' + m.id">{{ m.id }}</a><p>{{ m.revoked ? 'Key revoked' : m.standing }} · {{ m.balance }} TrustCOIN</p><small>{{ state.testNetwork ? 'Test identity' : 'GitHub ID ' + m.githubId }} · Operator {{ m.operator }}</small></article></div><p v-if="!state.members.length" class="empty">No contributor identities have been admitted. This screen displays ledger data, with no example contributors.</p></section>
