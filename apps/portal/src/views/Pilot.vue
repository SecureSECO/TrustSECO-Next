<template>
  <main class="pilot">
    ...
    <template v-if="state">
      ...
      <p class="note" v-if="state.policy.assignment === 'availability-beacon-v1'">{{ state.availableContributors }} contributors available. Three observers are assigned from a frozen availability pool using a verified randomness beacon. Missed deadlines activate reserves in a fixed order; disagreements never trigger replacements. <router-link to="/jobs">View assignment progress →</router-link></p>
      <section id="work"><h2>Observations &amp; trust scores</h2><p>These collectors measure the repository today. A version label associates the finding with a package; it does not reconstruct that version’s historical repository statistics.</p>
        ...
        <article class="round" v-for="round in [...state.rounds].reverse()" :key="round.id">
          ...
          <p v-if="round.assignment">{{ assignmentStatus(round) }}<br><small v-if="round.assignment.version === 'commit-reveal-v1'">Frozen pool: {{ round.assignment.pool.length }} operators · {{ Object.keys(round.assignment.commitments).length }} commitments · {{ Object.keys(round.assignment.reveals).length }} reveals. Measurement window starts {{ date(round.assignment.revealUntil) }}.</small></p>
...
const status = (r: any) => !r.closed ? (r.assignment && (!r.assignment.seed || (r.assignment.version === 'commit-reveal-v1' && state.value.at < r.assignment.revealUntil)) ? '● Preparing assignment' : '● Collecting') : r.result.status === 'verified' ? Number.isSafeInteger(r.confirmationHeight) && r.confirmationHeight <= state.value.ledger.finalizedHeight ? '✓ Community agreement · finalized' : '● Community agreement · recorded' : r.result.status === 'disputed' ? '● Conflicting observations' : '○ Not enough agreement';
const assignmentStatus = (r: any) => r.assignment.version === 'availability-beacon-v1' ? (!r.assignment.seed ? (r.closed ? 'Randomness beacon was unavailable; no replacement draw.' : 'Waiting for the fixed randomness beacon.') : 'Assigned: ' + r.assignment.committee.join(', ') + '. Reserves used: ' + (r.assignment.nextReserve - 3) + '.') : r.assignment.committee.length ? 'Assigned observers: ' + r.assignment.committee.join(', ') : state.value.at >= r.assignment.revealUntil ? 'Assignment failed: incomplete entropy. No replacement draw.' : 'Waiting for all frozen operators to complete the draw.';
