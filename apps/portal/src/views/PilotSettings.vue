  ...
  <main class="setup">
    ...
    <template v-else-if="state">
      ...
      <template v-if="state.identity">
        ...
        <section><h2>4. Automatic mining</h2><p>{{ state.activity }}</p><p v-if="state.lastSuccess">Last recorded observation: {{ new Date(state.lastSuccess).toLocaleString() }}</p><p v-if="!state.admitted">Mining becomes available after admission.</p><p v-if="state.revoked || state.standing === 'suspended'">This identity cannot mine while suspended or revoked.</p><button :disabled="busy || (!state.mining && (!state.admitted || state.revoked || state.standing === 'suspended'))" @click="toggle">{{ state.mining ? 'Stop mining' : 'Start mining' }}</button><p>Mining runs on this node even when you close this page. Stopping lets an in-flight submission finish.</p></section>
