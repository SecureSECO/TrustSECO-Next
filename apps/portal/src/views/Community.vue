 ...
 <main class="community">
  ...
  <template v-if="data">
   ...
   <section><h2>Contributors</h2><p class="muted">Only reviewed, substantiated incidents count. A disagreement never creates a strike automatically. Repeated reports of the same root cause or round count once.</p>
    <div class="members"><article v-for="m in data.members" :key="m.id"><h3>{{ m.id }}</h3><span :class="['badge',m.standing]">{{ m.standing === 'review' ? 'Needs review' : m.standing }}</span><p>{{ m.incidents }} incidents · {{ m.observations }} observations</p><p>{{ m.rewardEligibleObservations }} reward-eligible observations</p><small>Eligibility count only; no TrustCOIN paid.</small></article></div>
   ...
   <section><h2>Signed audit trail</h2><p class="muted">Every decision and appeal remains recorded. Suspended contributors may still submit observations, but those submissions cannot contribute to verification or TrustCOIN rewards.</p><ol><li v-for="a in [...data.audit].reverse()" :key="a.id"><strong>{{ a.kind }}</strong> · {{ a.actor }} · block {{ a.height }} <span class="badge">{{ a.height <= data.ledger.finalizedHeight ? 'Ledger finalized' : 'Awaiting ledger finality' }}</span><details><summary>Evidence and signature</summary><pre>{{ a.payload }}</pre><pre>{{ a.signature }}</pre></details></li></ol></section>
  </template>
 </main>
</template>
