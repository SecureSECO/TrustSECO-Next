<template>
 ...
 <p v-if="pilot" class="muted">Bounties are settled after a 24-hour review period following round closure. Unresolved work and division remainders return to the sponsor.</p>
 <p v-else class="muted">Payouts are stored permanently on the ledger and shown 200 at a time. Older balance changes were not recorded as payout history. Community reward eligibility is not a payment.</p>
 <p v-if="error" role="alert">{{ error }} <button @click="refresh(cursor)">Retry</button></p>
 <p v-else-if="!data">Loading payouts…</p>
 <p v-else-if="!data.payouts.length">{{ pilot ? 'No payouts recorded yet. Rewards become eligible 24 hours after a round closes.' : 'No payouts recorded yet. Mining payouts currently become eligible after 5,760 blocks—about 24 hours with 15-second blocks.' }}</p>
 ...
 <tr v-for="(p,i) in data.payouts" :key="`${p.height}:${p.jobID}:${p.uid}:${i}`"><td>{{ new Date(p.timestamp*1000).toLocaleString() }}</td><td>{{ p.uid }}</td><td>{{ p.package }} {{ p.version }}<small>{{ p.round ? "Round " + p.round + " · " + p.kind : "Job " + p.jobID }}</small></td><td><TrustCoinAmount :amount="p.amount" /></td><td>{{ p.height <= data.finalizedHeight ? 'Finalized' : 'Awaiting finality' }}<small>Block {{ p.height }}</small></td></tr>
