...
(async()=>{for(;;){try{
 ...
 if(!s.audit.every(a=>a.height<=s.ledger.finalizedHeight)){await sleep(15000);continue;}
 await durableEvent(url,keyFile,{kind:'open',round:p.roundID,package:p.repository,repository:p.repository,version:p.version,metric:'lib_sourcerank',source:'Libraries.io REST',method:'libraries-project-v1',packagePlatform:p.platform,packageName:p.name,duration:180,bounty:'300'});
